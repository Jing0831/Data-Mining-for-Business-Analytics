...
hc2<- hclust(d, method="ward.D2") #MOST BALANCE!
plot(hc2, main="Ward Method Dendrogram") # display dendogram

cutree(hc2,k=5)# k = number of clusters
mtcars$cluster = cutree(hc2,k=5)#create a cluster column
mtcars$cluster = NULL #REMOVES THE COLUMN
...
hc$cluster
aggregate(mtcars,by=list(hc$cluster),FUN=mean) #Combind
